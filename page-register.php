...
$errorMessage = false;
$updateMessage = false;
if(isset($_POST['email']) && isset($_POST['password']) && isset($_POST['cpassword']) && isset($_POST['fname']))
{
	if(!(isset($_POST['captcha']) && isset($_SESSION['secure_contact']) && $_SESSION['secure_contact'] == (int)$_POST['captcha']))
		...
		if($errorMessage == false)
		{
			...
			$userdata = array("ID" => '', "first_name" => $fname, "last_name" => $lname, "user_login" => $email, "user_email" => $email, "user_pass" => $password, "display_name" => $fname, "user_registered" => $rdate, "nickname" => $fname);
			$userID = wp_insert_user( $userdata );
			if($BBBThemeOptions->get_option('verify_email') == 'yes'){
				$verify_key = generate_random_int(12);
				$verify_url = get_permalink($BBBThemeOptions->get_option('page_login_id')).'?id='.$userID.'&vkey='.md5($verify_key);
				...
				$errorMessage = "Your account has been created successfully, And a verification email has been sent to your email address. Before you can login to your account you need to verify your email address. Visit your inbox and follow the instructions to verify your email address.";
				$updateMessage = true;
				/*wp_redirect(get_permalink($BBBThemeOptions->get_option('page_login_id')).'?mode=new');
				exit();*/
			}else{
			...
			unset($email); unset($password); unset($cpassword); unset($fname); unset($fname);
		}
}
get_header(); ?>
			...
			<?php if (have_posts()): while (have_posts()) : the_post(); ?>
				...
				<div class="web_boxp">
					...
					<div class="row">
						...
						<div class="col-xs-12 col-sm-12 col-md-8">
							<input type="email" name="email" id="email" class="required" required="required" <?php if(isset($fname)){ echo 'value="'.$email.'"';} ?> />
						</div>
					</div>
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-4">
					...
					<p>By creating an account, you confirm that you are agree to our <a href="<?php echo get_permalink($BBBThemeOptions->get_option('page_terms_of_use_id')); ?>">Terms of Use</a>.</p>
