	...
	if(isset($_SESSION['secure_contact']) && $_SESSION['secure_contact'] == (int)$_POST['captcha']){
		...
		if($email){
			...
			if($user_in){
				...
				$errorMessage = "An email has been sent on your email addrress. For further information visit your inbox.".$losturl;
				$updateMessage = true;
				if(isset($_GET['id']))
					unset($_GET['id']);
			}
	...
	if(strtotime("now") < $exptime && md5($db_key) === $key)
	{
		...
		if(isset($_POST['cpassword']) && isset($_POST['password']))
		{
			...
			if(strlen($password) >= 6 && $password == $cpassword)
			{
				...
				$verify_key = false;
				$errorMessage = 'Your password has been updated successfully. <a href="'.get_permalink($BBBThemeOptions->get_option('page_login_id')).'" style="color:#fff;">click here to login</a>';
				$updateMessage = true;
			}
			else
